...
<head>
    ...
    <title>Khôi Phục Quốc Duy ID</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro Display", sans-serif; -webkit-font-smoothing: antialiased; }
        ...
        input[type="email"] { width: 100%; height: 50px; padding: 0 16px; font-size: 15px; border: 1px solid #d2d2d7; border-radius: 12px; outline: none; transition: 0.2s; }
...
<body>
    <div class="card">
        ...
        <p class="subtitle">Nhập địa chỉ email liên kết với Quốc Duy ID để nhận mã OTP đặt lại mật khẩu</p>
        <c:if test="${not empty error}">
            <div class="error-message">${error}</div>
        </c:if>
        <form action="<c:url value='/forgot-password'/>" method="post">
            <div class="input-group">
                <input type="email" name="email" placeholder="Nhập địa chỉ email của bạn" required autofocus>
            </div>
            <button type="submit" class="btn-submit">Gửi mã khôi phục</button>
        </form>
